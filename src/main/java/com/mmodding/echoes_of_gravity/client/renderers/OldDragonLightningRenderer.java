...
public class OldDragonLightningRenderer extends EntityRenderer<OldDragonLightningEntity> {

	public OldDragonLightningRenderer(EntityRendererFactory.Context context) {
		super(context);
	}

	public void render(OldDragonLightningEntity oldDragonLightningEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
		float[] fs = new float[8];
		float[] gs = new float[8];
		float h = 0.0F;
		float j = 0.0F;
		Random random = new Random(oldDragonLightningEntity.seed);
		...
		for (int l = 0; l < 4; ++l) {
			Random random2 = new Random(oldDragonLightningEntity.seed);
